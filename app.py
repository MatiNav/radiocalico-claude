...
def get_ratings(song_id):
    ...
    thumbs_up = cursor.execute(
        'SELECT COUNT(*) FROM ratings WHERE song_id = ? AND rating = 1',
        (song_id,)
    ).fetchone()[0]

    # Get thumbs down count
    thumbs_down = cursor.execute(
        'SELECT COUNT(*) FROM ratings WHERE song_id = ? AND rating = 0',
        (song_id,)
    ).fetchone()[0]
...
def rate_song(song_id):
    ...
    if rating not in [0, 1]:
        return jsonify({'error': 'rating must be 0 (thumbs down) or 1 (thumbs up)'}), 400
...
@app.route('/')
def index():
    """Serve the radio player homepage."""
